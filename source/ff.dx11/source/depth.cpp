...
ff::dx11::depth::depth(const ff::point_size& size, size_t sample_count)
{
    ...
    texture_desc.Width = static_cast<UINT>(std::max<size_t>(size.x, 1));
    texture_desc.Height = static_cast<UINT>(std::max<size_t>(size.y, 1));
...
ff::point_size ff::dx11::depth::size() const
{
...
bool ff::dx11::depth::size(const ff::point_size& size)
{
    if (this->size() != size)
    {
        ff::dx11::depth new_depth(size, this->sample_count());
        if (!new_depth)
        {
            assert(false);
            return false;
