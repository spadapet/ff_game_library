...
static internal_ff_arena_buffer* new_virtual_buffer(size_t size, bool oversize)
{
    ...
    size_t actual_size = ff_round_up(size, get_allocation_granularity());

    if (oversize)
    {
        // Oversize: single VirtualAlloc, header + entire payload reserved AND committed.
        internal_ff_arena_buffer* new_buffer = (internal_ff_arena_buffer*)VirtualAlloc(nullptr, actual_size, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
        FF_ASSERT_RET_VAL(new_buffer, nullptr);
    ...
    internal_ff_arena_buffer* new_buffer = (internal_ff_arena_buffer*)VirtualAlloc(nullptr, actual_size, MEM_RESERVE, PAGE_READWRITE);
    FF_ASSERT_RET_VAL(new_buffer, nullptr);

    size_t initial_commit = get_page_size();
    initial_commit = ff_min(initial_commit, actual_size);
...
static internal_ff_arena_buffer* init_external_buffer(void* data, size_t size)
{
    ...
    uint8_t* header = ff_align_up((uint8_t*)data, alignof(internal_ff_arena_buffer));
...
static bool commit_virtual_buffer(internal_ff_arena_buffer* buffer, uint8_t* needed_end)
{
    ...
    {
        target = ff_min(ff_round_up_pow2(needed_total), reserve_total);
    }
...
void ff_arena_init_external(void* buffer, size_t size, size_t grow_buffer_size)
{
    ...
    this->grow_buffer_size = ff_round_up_pow2(ff_max(initial_grow, page_size));
    this->max_buffer_size = ff_max(this->grow_buffer_size, s_max_heap_buffer_size);
    this->spare = nullptr;
    this->type = internal_ff_arena_type_heap_global;
...
void ff_arena_init_heap(size_t initial_buffer_size)
{
    size_t page_size = page_size();

    this->next = nullptr;
    this->end = nullptr;
    this->grow_buffer_size = ff_round_up_pow2(ff_max(initial_buffer_size, page_size));
    this->max_buffer_size = ff_max(this->grow_buffer_size, max_heap_buffer_size);
    this->buffer = nullptr;
    this->spare = nullptr;
    this->type = internal_ff_arena_type_heap;
    this->heap = GetProcessHeap();

    internal_ff_arena_buffer* new_buffer = new_heap_buffer(this->heap, this->grow_buffer_size, false);
...
void ff_arena_init_heap_local(size_t initial_buffer_size)
{
    this->next = nullptr;
    this->end = nullptr;
    ...
    this->buffer = nullptr;
    this->spare = nullptr;
    this->type = internal_ff_arena_type_heap_local;
...
void ff_arena_init_virtual_memory(size_t initial_buffer_size)
{
    ...
    this->end = nullptr;
    this->heap = GetProcessHeap();
    this->grow_buffer_size = ff_round_up_pow2(ff_max(initial_buffer_size, allocation_granularity));
    // Virtual reservations are cheap (lazy commit), so cap much higher than heap; honor a larger user-requested initial size.
    this->max_buffer_size = ff_max(this->grow_buffer_size, max_virtual_buffer_size);
    this->buffer = nullptr;
    this->spare = nullptr;
    this->type = internal_ff_arena_type_virtual_memory;

    internal_ff_arena_buffer* new_buffer = ::new_virtual_buffer(this->grow_buffer_size, false);
    FF_ASSERT_RET(new_buffer);
...
void ff_arena_destroy()
{
    if (this->type == internal_ff_arena_type::heap_local)
    {
...
{
    uint8_t* aligned = ff_align_up(arena->next, align);
    ...
    internal_ff_arena_buffer* new_buffer = nullptr;
    ...
    if (!new_buffer && arena->grow_buffer_size > 0)
    {
        ...
        else
        {
            ...
            alloc_size = ff_round_up_pow2(ff_max(arena->grow_buffer_size, needed));
        }
    ...
    aligned = ff_align_up(arena->next, align);
    uint8_t* needed_end = aligned + size;
...
void* ff_arena_alloc(size_t size, size_t align)
{
    ...
    uint8_t* aligned = ff_align_up(this->next, align);
...
void* ff_arena_realloc(const void* start, size_t size, size_t new_size, size_t align)
{
    FF_ASSERT(ff::is_pow2(align));
    ...
    if (new_start && size)
    {
        memcpy(new_start, start, ff_min(size, new_size));
    }
...
void ff_arena_reset()
{
    ...
    internal_ff_arena_buffer* external_buffer = nullptr;
    internal_ff_arena_buffer* largest = nullptr;
    size_t largest_size = 0;

    for (internal_ff_arena_buffer* current_buffer = this->buffer; current_buffer; current_buffer = current_buffer->next)
    {
    ...
    for (internal_ff_arena_buffer* current_buffer = this->spare; current_buffer; current_buffer = current_buffer->next)
    {
    ...
    internal_ff_arena_buffer* current_buffer = this->buffer;
...
ff_arena_marker ff_arena_mark() const
{
    ff_arena_marker marker;
    marker.next = this->next;
    return marker;
}

void ff_arena_rewind(ff_arena_marker marker)
{
    ...
    {
        internal_ff_arena_buffer* old_buffer = this->buffer;
