...
typedef bool(*ff_assert_listener_func)(const char* exp, const char* text, const char* file, unsigned int line);
...
{ \
    if(!(exp) && !ff_internal_assert_core(#exp, txt, __FILE__, __LINE__)) \
    { \
        __debugbreak(); \
    } \
}
