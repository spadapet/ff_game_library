...
static bool parse_object(internal_ff_json_parser* parser, ff_dict* dict)
{
    ...
    while (token.type != ff_json_token_type_close_curly)
    {
        ...
        // and last-wins matches both the old C++ parser and what most JSON parsers do). ff_dict_add
        // gets that for free, because a lookup searches newest first, and it avoids the scan through
        // every existing entry that ff_dict_set needs to delete the earlier one.
        ff_dict_add(dict, ff_value_as_string(&key), &value);
...
bool ff_json_parse_idict(ff_string_view text, ff_idict* dict, ff_arena* arena, const char** error_pos)
{
    ...
    // A private heap keeps the mutable dict and its decoded strings out of the caller's arena, so
    // only the finished block is left behind. It all goes away in one call.
    ff_arena scratch;
