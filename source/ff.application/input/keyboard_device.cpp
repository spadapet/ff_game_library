...
void ff::keyboard_device::notify_window_message(ff::window* window, ff::window_message& message)
{
    ...
    switch (message.msg)
    {
        ...
        case WM_KEYDOWN:
            ...
            {
                int count = static_cast<int>(message.lp & 0xFFFF);
                unsigned int vk = static_cast<unsigned int>(message.wp);
                this->device_event.notify(ff::input_device_event_key_press(vk, count));
