...
void ff::state::render(ff::target_base& target, ff::dxgi::depth_base& depth)
{
    for (size_t i = 0; i < this->child_state_count(); i++)
    {
...
void ff::state::frame_rendered(ff::state::advance_t type, ff::target_base& target, ff::dxgi::depth_base& depth)
{
    for (size_t i = 0; i < this->child_state_count(); i++)
    {
