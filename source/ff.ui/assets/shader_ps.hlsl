...
Texture2D<uint> paletteImage: register(t5);
Texture2D palette: register(t6);
...
cbuffer Buffer3: register(b2)
{
    float4 patternSize;
    uint4 paletteRow;
}

float4 SamplePattern(float2 uv)
{
    float4 color1 = pattern.Sample(patternSampler, uv);
    uint index = paletteImage.Load(int3(uv * patternSize.xy, 0));
    float4 color2 = palette.Load(int3(index, paletteRow[0], 0));

    return color1 + color2;
}

float4 SamplePatternGrad(float2 uv, float2 gx, float2 gy)
{
    float4 color1 = pattern.SampleGrad(patternSampler, uv, gx, gy);
    uint index = paletteImage.Load(int3(uv * patternSize.xy, 0));
    float4 color2 = palette.Load(int3(index, paletteRow[0], 0));
...
Out main(in In i)
{
    ...
    /////////////////////////////////////////////////////
        ...
        half opacity_ = radialGrad0.w;
            ...
            half4 paint = inside * SamplePattern(i.uv0);
            ...
            half4 paint = inside * SamplePatternGrad(uv, ddx(i.uv0), ddy(i.uv0));
        #else
            half4 paint = SamplePattern(i.uv0);
        #endif
        half opacity_ = opacity;
    #endif
    ...
    /////////////////////////////////////////////////////
        ...
        o.color = lerp(image.Sample(imageSampler, i.uv1), SamplePattern(i.uv0), paint.a);
