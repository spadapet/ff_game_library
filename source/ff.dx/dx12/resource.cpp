...
void ff::dx12::resource::create_shader_view(D3D12_CPU_DESCRIPTOR_HANDLE view, size_t array_start, size_t array_count, size_t mip_start, size_t mip_count)
{
    const D3D12_RESOURCE_DESC& texture_desc = this->desc();
    ...
    ff::dx12::device()->CreateShaderResourceView(ff::dx12::get_resource(*this), &view_desc, view);
}

void ff::dx12::resource::create_target_view(D3D12_CPU_DESCRIPTOR_HANDLE view, size_t array_start, size_t array_count, size_t mip_level)
{
    const D3D12_RESOURCE_DESC& texture_desc = this->desc();
    ...
    ff::dx12::device()->CreateRenderTargetView(ff::dx12::get_resource(*this), &view_desc, view);
}
...
bool ff::dx12::resource::reset()
{
    ...
    else if (!this->mem_range_)
    {
        ...
        if (FAILED(ff::dx12::device()->CreateCommittedResource(
            ...
            FAILED(this->resource_.As(&pageable)))
        {
            return false;
        }
