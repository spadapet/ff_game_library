...
namespace ff
{
    ...
    {
    public:
        using this_type = typename unordered_set<Key, Hash, KeyEqual>;
        using internal_type = typename ff::internal::hash_set<Key, Hash, KeyEqual, false, true>;
        ...
        unordered_set(std::initializer_list<value_type> init, size_type bucket_count = 0)
            : data(init, bucket_count)
        {
        }

        unordered_set(const this_type& other)
            : data(other.data)
        {
        }

        unordered_set(this_type&& other)
        ...
        {
            return this->data.insert(hint, value);
        }

        std::pair<iterator, bool> insert(const_iterator hint, value_type&& value)
        {
            return this->data.insert(hint, std::move(value));
        }
...
namespace std
{
    ...
    void swap(ff::unordered_set<Key, Hash, KeyEqual>& lhs, ff::unordered_set<Key, Hash, KeyEqual>& other) noexcept(noexcept(lhs.swap(other)))
    {
        lhs.swap(other);
    }
}
