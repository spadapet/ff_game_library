...
namespace data_test
{
    ...
    public:
        TEST_METHOD(compress_file)
        {
            ...
            auto com_spec_data = std::make_shared<ff::data_mem_mapped>(ff::file_mem_mapped(com_spec_string));
            auto compress_vector = std::make_shared<std::vector<uint8_t>>();
            auto uncompress_vector = std::make_shared<std::vector<uint8_t>>();

            // Compress
            {
                ff::data_reader com_spec_reader(com_spec_data);
                compress_vector->reserve(com_spec_reader.size() / 2);
                uncompress_vector->reserve(com_spec_reader.size());

                ff::data_writer writer(compress_vector);
                bool status = ff::compression::compress(com_spec_reader, com_spec_reader.size(), writer);
            ...
            // Decompress
            {
                std::shared_ptr<ff::data_vector> compress_data = std::make_shared<ff::data_vector>(compress_vector);
                ff::data_reader compress_reader(compress_data);
                ff::data_writer uncompress_writer(uncompress_vector);

                bool status = ff::compression::uncompress(compress_reader, compress_vector->size(), uncompress_writer);
