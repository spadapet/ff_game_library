...
// Tests for ff_dispatch: a per-thread work queue drained by a message window.
// Coverage:
...
namespace ff::test::base
{
    ...
    static bool arrays_distinct(const ff_dispatch* dispatch)
    {
    ...
    static ff_dispatch chain_dispatch;
    ...
    static void chain_post(void* cookie)
    {
        ...
        {
            ff_dispatch_post(&chain_dispatch, chain_post, cookie);
        }
    }
    ...
    static void chain_post_and_flush(void* cookie)
    {
        ...
        if (++chain_ran < chain_limit)
        {
            ff_dispatch_post(&chain_dispatch, chain_post_and_flush, cookie);
            ff_dispatch_flush(&chain_dispatch);
        }
    }

    TEST_CLASS(dispatch_tests)
    {
    public:
        ...
        TEST_METHOD(post_runs_on_flush)
        {
            ...
            counter state{};
            ff_dispatch_post(&dispatch, count_call, &state);
        ...
        TEST_METHOD(send_on_owning_thread_runs_inline)
        {
            ...
            counter state{};
            ff_dispatch_send(&dispatch, count_call, &state);
            Assert::AreEqual(1l, state.calls);
        ...
        TEST_METHOD(entries_run_once_and_in_order)
        {
            ...
            {
                ff_dispatch_post(&dispatch, [](void* cookie)
                {
        ...
        {
            Assert::IsTrue(ff_dispatch_init(&chain_dispatch, ff_dispatch_type_none));

            chain_ran = 0;
            chain_limit = 50;

            // Work posted from inside a drain has to be picked up by the same drain loop.
            ff_dispatch_post(&chain_dispatch, chain_post, nullptr);
            ff_dispatch_flush(&chain_dispatch);

            Assert::AreEqual(50l, chain_ran);
            Assert::IsTrue(arrays_distinct(&chain_dispatch));

            ff_dispatch_destroy(&chain_dispatch);
        }
        ...
        TEST_METHOD(nested_flush_runs_each_entry_once)
        {
            ...
            ff_dispatch_post(&dispatch, [](void*)
            {
                ...
                if (++guard == 1)
                {
                    ff_dispatch_post(&dispatch, [](void*) { inner++; }, nullptr);
                    ff_dispatch_flush(&dispatch);
                }
            }, nullptr);

            ff_dispatch_post(&dispatch, [](void*) { sibling++; }, nullptr);
            ff_dispatch_flush(&dispatch);
        ...
        TEST_METHOD(deep_nested_flush_does_not_grow_stack)
        {
            ...
            ff_dispatch_post(&chain_dispatch, chain_post_and_flush, nullptr);
            ff_dispatch_flush(&chain_dispatch);
        ...
        TEST_METHOD(destroy_from_callback)
        {
            ...
            ff_dispatch_post(&dispatch, [](void*) { ff_dispatch_destroy(&dispatch); }, nullptr);
            ff_dispatch_post(&dispatch, [](void*) { tail++; }, nullptr);
            ff_dispatch_post(&dispatch, [](void*) { tail++; }, nullptr);
        ...
        TEST_METHOD(post_from_many_threads)
        {
            ...
            for (intptr_t i = 0; i < thread_count; i++)
            {
                threads[i] = ::CreateThread(nullptr, 0, [](void* cookie) -> DWORD
                {
                    ...
                    {
                        ff_dispatch_post(&dispatch, [](void* inner)
                        {
        ...
        TEST_METHOD(typed_dispatch_is_cached_and_released)
        {
            Assert::IsNull(ff_dispatch_get_main());
            Assert::IsNull(ff_dispatch_get_game());

            ff_dispatch main_dispatch;
            ff_dispatch game_dispatch;
            Assert::IsTrue(ff_dispatch_init(&main_dispatch, ff_dispatch_type_main));
            Assert::IsTrue(ff_dispatch_init(&game_dispatch, ff_dispatch_type_game));

            Assert::IsTrue(&main_dispatch == ff_dispatch_get_main());
            Assert::IsTrue(&game_dispatch == ff_dispatch_get_game());

            ff_dispatch_destroy(&main_dispatch);
            Assert::IsNull(ff_dispatch_get_main());
            Assert::IsTrue(&game_dispatch == ff_dispatch_get_game());

            ff_dispatch_destroy(&game_dispatch);
            Assert::IsNull(ff_dispatch_get_game());
        }

        TEST_METHOD(destroy_from_callback_unregisters_immediately)
        {
            static ff_dispatch dispatch;
            Assert::IsTrue(ff_dispatch_init(&dispatch, ff_dispatch_type_main));

            static bool visible_after_destroy;
            visible_after_destroy = true;

            // Destroying from a callback defers teardown until the drain finishes, so the cached
            // pointer has to be dropped by destroy itself rather than by the deferred teardown.
            ff_dispatch_post(&dispatch, [](void*)
            {
                ff_dispatch_destroy(&dispatch);
                visible_after_destroy = (ff_dispatch_get_main() != nullptr);
            }, nullptr);

            ff_dispatch_flush(&dispatch);

            Assert::IsFalse(visible_after_destroy);
            Assert::IsNull(ff_dispatch_get_main());
        }

        TEST_METHOD(untyped_dispatch_is_not_cached)
        {
            ff_dispatch dispatch;
            Assert::IsTrue(ff_dispatch_init(&dispatch, ff_dispatch_type_none));
        ...
        TEST_METHOD(current_dispatch_is_per_thread)
        {
            static ff_dispatch dispatch;
            Assert::IsTrue(ff_dispatch_init(&dispatch, ff_dispatch_type_main));
            Assert::IsTrue(&dispatch == ff_dispatch_get_current());

            // Another thread never initialized one, so its slot stays empty.
            static bool other_thread_null;
            other_thread_null = false;

            HANDLE thread = ::CreateThread(nullptr, 0, [](void*) -> DWORD
            {
                other_thread_null = (ff_dispatch_get_current() == nullptr);
                return 0;
            }, nullptr, 0, nullptr);

            Assert::IsNotNull(thread);
            Assert::AreEqual(WAIT_OBJECT_0, ::WaitForSingleObject(thread, 20000));
            Assert::IsTrue(other_thread_null);
            ::CloseHandle(thread);

            ff_dispatch_destroy(&dispatch);
            Assert::IsNull(ff_dispatch_get_current());
        }
    };
}
