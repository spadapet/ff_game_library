...
// Tests for ff_hash_bytes / ff_hash_string / ff_hash_data (a streaming wyhash variant).
// Coverage:
...
static uint64_t hash_two_parts(const uint8_t* p, size_t size, size_t split)
{
    ff_hash_data state;
    ff_hash_init(&state);
    ff_hash(&state, p, split);
    ff_hash(&state, p + split, size - split);
...
namespace ff::test::base
{
    ...
    public:
        ...
        TEST_METHOD(hash_bytes_is_deterministic)
        {
            ...
            Assert::AreEqual<uint64_t>(ff_hash_bytes(buffer, sizeof(buffer)), ff_hash_bytes(buffer, sizeof(buffer)));
        }

        TEST_METHOD(hash_string_matches_hash_bytes)
        {
            const char text[] = "dictionary key";
            Assert::AreEqual<uint64_t>(ff_hash_bytes(text, sizeof(text) - 1), ff_hash_string(FF_SVL("dictionary key")));
        }

        TEST_METHOD(hash_wstring_matches_hash_bytes)
        {
            const wchar_t text[] = L"dictionary key";
            Assert::AreEqual<uint64_t>(ff_hash_bytes(text, sizeof(text) - sizeof(wchar_t)), ff_hash_wstring(FF_WSVL(L"dictionary key")));
        }

        TEST_METHOD(narrow_and_wide_hash_differ)
        {
            // Same characters, different byte width -> different bytes -> different hashes.
            Assert::AreNotEqual<uint64_t>(ff_hash_string(FF_SVL("hello")), ff_hash_wstring(FF_WSVL(L"hello")));
        }

        TEST_METHOD(empty_input_is_stable_and_consistent)
        {
            uint64_t a = ff_hash_bytes(nullptr, 0);
            uint64_t b = ff_hash_bytes("ignored", 0);
            uint64_t c = ff_hash_string(FF_SVL(""));

            ff_hash_data state;
            ff_hash_init(&state);
            uint64_t d = ff_hash_done(&state); // no hash() calls at all
        ...
        {
            uint64_t h_a = ff_hash_string(FF_SVL("a"));
            uint64_t h_b = ff_hash_string(FF_SVL("b"));
            uint64_t h_ab = ff_hash_string(FF_SVL("ab"));
            uint64_t h_ba = ff_hash_string(FF_SVL("ba"));
        ...
        TEST_METHOD(streamed_equals_one_shot_at_every_split)
        {
            ...
            uint64_t one_shot = ff_hash_bytes(buffer, sizeof(buffer));
        ...
        TEST_METHOD(streamed_equals_one_shot_byte_by_byte)
        {
            ...
            for (size_t li = 0; li < sizeof(lengths) / sizeof(lengths[0]); li++)
            {
                ...
                uint64_t one_shot = ff_hash_bytes(buffer, len);

                ff_hash_data state;
                ff_hash_init(&state);
                for (size_t i = 0; i < len; i++)
                {
                    ff_hash(&state, buffer + i, 1);
                }
                Assert::AreEqual<uint64_t>(one_shot, ff_hash_done(&state), L"byte-by-byte streamed hash differed from one-shot");
            }
        }
        ...
        TEST_METHOD(streamed_equals_one_shot_three_chunks)
        {
            ...
            for (size_t a = 0; a <= sizeof(buffer); a += 5)
            {
                for (size_t b = a; b <= sizeof(buffer); b += 7)
                {
                    ff_hash_data state;
                    ff_hash_init(&state);
                    ff_hash(&state, buffer, a);
                    ff_hash(&state, buffer + a, b - a);
                    ff_hash(&state, buffer + b, sizeof(buffer) - b);
                    Assert::AreEqual<uint64_t>(one_shot, ff_hash_done(&state));
                }
            }
        }
        ...
        TEST_METHOD(single_bit_flip_avalanches_output)
        {
            ...
            uint64_t base = ff_hash_bytes(buffer, sizeof(buffer));
            ...
            for (int bit = 0; bit < total_bits; bit++)
            {
                ...
                uint64_t flipped = ff_hash_bytes(buffer, sizeof(buffer));
        ...
        TEST_METHOD(low_bits_uniform_for_dictionary_keys)
        {
            ...
            for (int i = 0; i < key_count; i++)
            {
                ...
                uint64_t h = ff_hash_bytes(key, (size_t)n);
        ...
        {
            uint64_t h1 = ff_hash_string(FF_SVL("player_health"));
            uint64_t h2 = ff_hash_string(FF_SVL("player_healht")); // last two chars transposed
            uint64_t h3 = ff_hash_string(FF_SVL("player_health ")); // one extra byte
        ...
        TEST_METHOD(no_collisions_across_many_keys)
        {
            ...
            for (int i = 0; i < key_count; i++)
            {
                ...
                hashes[i] = ff_hash_bytes(key, (size_t)n);
            }
        ...
        TEST_METHOD(length_changes_hash)
        {
            ...
            {
                hashes[len] = ff_hash_bytes(buffer, (size_t)len);
            }
            for (int a = 1; a <= 64; a++)
            {
        ...
        TEST_METHOD(known_hash_values_are_stable)
        {
            ...
            Assert::AreEqual<uint64_t>(0x0409638EE2BDE459ull, ff_hash_bytes("", 0));
            Assert::AreEqual<uint64_t>(0x28D2053309D28531ull, ff_hash_string(FF_SVL("a")));
            Assert::AreEqual<uint64_t>(0x0E24BBD9F93F532Dull, ff_hash_string(FF_SVL("hello")));
            Assert::AreEqual<uint64_t>(0x668D5E431C3B2573ull, ff_hash_string(FF_SVL("hello world")));
            Assert::AreEqual<uint64_t>(0xC304E72C387CD229ull, ff_hash_string(FF_SVL("0123456789abcdef")));
            Assert::AreEqual<uint64_t>(0xEACB223E285616F0ull, ff_hash_string(FF_SVL("0123456789abcdefg")));
            Assert::AreEqual<uint64_t>(0xF88C518CF1EFA0BAull, ff_hash_string(FF_SVL("The quick brown fox jumps over the lazy dog")));
        }
    };
}
