...
namespace ff::test::base
{
    ...
    public:
        ...
        TEST_METHOD(realloc_shrink_in_place_when_last_block)
        {
            ...
            uint8_t* p = (uint8_t*)arena.alloc(128, 8);
            Assert::IsNotNull(p);
            fill_pattern(p, 128, 0x22);
            Assert::IsTrue(arena.next == p + 128);

            // Shrink the last block: same address, next moves down.
            uint8_t* shrunk = (uint8_t*)arena.realloc(p, 128, 32, 8);
        ...
        TEST_METHOD(realloc_equal_size_in_place_is_noop_address)
        {
            ...
            uint8_t* p = (uint8_t*)arena.alloc(48, 8);
            Assert::IsNotNull(p);
            Assert::IsTrue(arena.next == p + 48);

            uint8_t* same = (uint8_t*)arena.realloc(p, 48, 48, 8);
        ...
        TEST_METHOD(realloc_grow_in_place_frees_tail_space)
        {
            ...
            uint8_t* p = (uint8_t*)arena.alloc(256, 8);
            Assert::IsNotNull(p);
            uint8_t* shrunk = (uint8_t*)arena.realloc(p, 256, 64, 8);
            Assert::IsTrue(shrunk == p);

            void* next_alloc = arena.alloc(8, 8);
        ...
        TEST_METHOD(realloc_relocates_when_grow_overflows_buffer)
        {
            ...
            uint8_t* moved = (uint8_t*)arena.realloc(p, 64, 1024 * 1024, 8);
        ...
        TEST_METHOD(realloc_shrink_copies_only_new_size_when_relocated)
        {
            ...
            uint8_t* first = (uint8_t*)arena.alloc(128, 8);
            Assert::IsNotNull(first);
            fill_pattern(first, 128, 0x66);

            // Make 'first' not the last block so a shrink relocates.
            uint8_t* second = (uint8_t*)arena.alloc(16, 8);
            Assert::IsNotNull(second);

            uint8_t* moved = (uint8_t*)arena.realloc(first, 128, 32, 8);
            Assert::IsTrue(moved != first);
        ...
        TEST_METHOD(realloc_preserves_alignment_on_relocate)
        {
            ...
            uint8_t* first = (uint8_t*)arena.alloc(64, 8);
            Assert::IsNotNull(first);
            uint8_t* second = (uint8_t*)arena.alloc(8, 8); // make 'first' not last
            Assert::IsNotNull(second);

            // Relocate with a stricter alignment requirement.
            void* moved = arena.realloc(first, 64, 200, 64);
            Assert::IsNotNull(moved);
        ...
        TEST_METHOD(realloc_relocates_when_last_block_underaligned)
        {
            ...
            uint8_t* p = (uint8_t*)arena.alloc(8, 8);
            Assert::IsNotNull(p);
            while (is_aligned(p, 16))
            {
                p = (uint8_t*)arena.alloc(8, 8);
                Assert::IsNotNull(p);
            }
            ...
            uint8_t* moved = (uint8_t*)arena.realloc(p, 8, 64, 16);
            Assert::IsNotNull(moved);
            Assert::IsTrue(moved != p);
        ...
        TEST_METHOD(realloc_with_zero_new_size_returns_null)
        {
            ...
            void* result = arena.realloc(p, 64, 0, 8);
        ...
        TEST_METHOD(realloc_with_zero_old_size_acts_as_alloc)
        {
            ...
            void* result = arena.realloc(nullptr, 0, 64, 8);
        ...
        TEST_METHOD(realloc_of_foreign_pointer_copies_into_arena)
        {
            ...
            uint8_t* moved = (uint8_t*)arena.realloc(source, sizeof(source), 256, 8);
            Assert::IsNotNull(moved);
        ...
        TEST_METHOD(realloc_repeated_in_place_growth)
        {
            ...
            uint8_t* p = (uint8_t*)arena.alloc(16, 8);
            Assert::IsNotNull(p);
            ...
            {
                uint8_t* grown = (uint8_t*)arena.realloc(p, current, target, 8);
        ...
        TEST_METHOD(rewind_after_inplace_realloc_restores_next)
        {
            ...
            arena.alloc(64, 8); // some prior content
            ff::arena_marker m = arena.mark();

            // Allocate then grow the block in place (it's the last block).
            uint8_t* p = (uint8_t*)arena.alloc(32, 8);
            Assert::IsNotNull(p);
            uint8_t* grown = (uint8_t*)arena.realloc(p, 32, 128, 8);
            Assert::IsTrue(grown == p);
            Assert::IsTrue(arena.next == p + 128);
            Assert::IsTrue(arena.next != m.next);
        ...
        TEST_METHOD(rewind_after_realloc_relocates_unwinds_new_buffer)
        {
            ...
            arena.alloc(64, 8);
            ff::arena_marker m = arena.mark();
            const ff::internal::arena_buffer* marker_buffer = arena.buffer;
        ...
        TEST_METHOD(nested_mark_rewind_lifo)
        {
            ...
            uint8_t* base = (uint8_t*)arena.alloc(64, 8);
            Assert::IsNotNull(base);

            ff::arena_marker outer = arena.mark();
            void* outer_first = arena.alloc(104, 8);
            Assert::IsTrue(outer_first == outer.next);

            ff::arena_marker inner = arena.mark();
            void* inner_first = arena.alloc(104, 8);
            Assert::IsTrue(inner_first == inner.next);

            // Unwind the inner scope; allocating again reuses the inner address.
            arena.rewind(inner);
            Assert::IsTrue(arena.next == inner.next);
            void* inner_again = arena.alloc(104, 8);
            Assert::IsTrue(inner_again == inner_first);

            // Unwind the outer scope; allocating again reuses the outer address.
            arena.rewind(outer);
            Assert::IsTrue(arena.next == outer.next);
            void* outer_again = arena.alloc(104, 8);
        ...
        TEST_METHOD(nested_mark_rewind_lifo_across_growth)
        {
            ...
            arena.alloc(256, 8); // now in a later buffer
            ...
            arena.rewind(inner);
            Assert::IsTrue(arena.next == inner.next);
            Assert::IsTrue(inner.next >= arena.buffer->start && inner.next <= arena.buffer->end);

            arena.rewind(outer);
            Assert::IsTrue(arena.next == outer.next);
            Assert::IsTrue(outer.next >= arena.buffer->start && outer.next <= arena.buffer->end);
        ...
        TEST_METHOD(realloc_grow_in_place_to_exact_buffer_end)
        {
            ...
            uint8_t* grown = (uint8_t*)arena.realloc(p, 64, to_end, 8);
            ...
            const ff::internal::arena_buffer* head_before = arena.buffer;
            void* overflow = arena.alloc(1, 8);
        ...
        TEST_METHOD(realloc_grow_one_past_buffer_end_relocates)
        {
            ...
            size_t to_end = (size_t)(arena.end - p);
            uint8_t* moved = (uint8_t*)arena.realloc(p, 64, to_end + 1, 8);
            Assert::IsNotNull(moved);
            Assert::IsTrue(moved != p);
        ...
        TEST_METHOD(alloc_next_continues_after_inplace_realloc_grow)
        {
            ...
            uint8_t* grown = (uint8_t*)arena.realloc(p, 32, 96, 8);
            Assert::IsTrue(grown == p);

            uint8_t* after = (uint8_t*)arena.alloc(8, 8);
        ...
        TEST_METHOD(realloc_on_external_block_grows_in_place_then_relocates)
        {
            ...
            uint8_t* moved = (uint8_t*)arena.realloc(grown, 128, 4096, 8);
            Assert::IsNotNull(moved);
        ...
        TEST_METHOD(oversize_threshold_boundary_below_and_above_cap)
        {
            ...
            // (max_buffer_size for a small init_heap is the 1 MB default.)
            {
                ...
                void* under = arena.alloc(1024 * 1024 - 4096, 8);
            ...
            }
            {
                ...
                void* over = arena.alloc(1024 * 1024 + 4096, 8);
        ...
        TEST_METHOD(realloc_page_sized_alignment_relocate)
        {
            ...
            uint8_t* moved = (uint8_t*)arena.realloc(first, 64, 200, 4096);
            Assert::IsNotNull(moved);
